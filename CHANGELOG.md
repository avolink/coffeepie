...
### Token Economics
- **Parking Fee for dormant Slices** — powered-off/suspended Slices still reserve SSD/HDD on a provider's node, so they now accrue a holding fee. Consumers pay **10 Cr per dormant Slice per hour** (a small holding rate — dormant Slices use no compute, only reserved storage), free for the first 9 dormant Slices per account, applied from the 10th and up. Providers earn **1.5 COFP per dormant Slice per hour** (vs. 60 COFP/hour active), sized to cover storage cost plus Tier IV margin. Rate is a governance parameter, not a hardcoded constant (see `BACKEND_INFRA_AUDIT.md` Finding #45). Documented in `PROVIDERS.md`, `README.md`, `CONSTITUTION.md`, `blockchain/README.md`, `cloud-providers/README.md`.
- **Free-tier Credit lifecycle** — ad-earned Cr are credited **1:1 from the advertiser bid** (no markup, simpler taxation) and **expire 1 hour after grant** (unused Cr forfeited/breakage). Free machines consume **30 Cr/slice·min**. Purchased Credit Packages carry **1-week / 1-month / 1-year** validity. Documented in `README.md`, `blockchain/README.md`, `CONSTITUTION.md`.
- **Rush Hour Balancing Rates (Free tier only)** — demand-based load shaping: during peak windows the Free-tier compute rate scales with live regional utilization from off-peak **30 Cr/slice·min** up to **~1'000 Cr/slice·min** (Free Credits are ad-earned, so peak use costs more Ads). Paid tiers are not surged; customer-side only (provider COFP earning unchanged). Basic access never denied — Free degrades gracefully, not priced out. Governance/region-tunable. Documented in `README.md`, `SCHEDULING.md`.
