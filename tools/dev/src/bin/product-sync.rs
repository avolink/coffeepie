...
// Syncs Avo store product pages with productos.json data file.
//
// Reads product HTML pages from productos/ directory, extracts metadata,
// compares with existing productos.json, and reports changes.
//
// Usage:
//   product-sync                          # Dry-run: show what would change
//   product-sync --apply                  # Apply changes to productos.json
...
#[command(about = "Coffee Pie Product Catalog Sync — Avo ↔ productos.json", long_about = None)]
struct Cli {
    /// Product pages directory
    #[arg(long, default_value = "coffeepie_website/public/productos")]
    pages_dir: PathBuf,

    /// productos.json path
    #[arg(long, default_value = "coffeepie_website/public/data/productos.json")]
    data_file: PathBuf,

    /// Apply changes (writes to productos.json)
    #[arg(long)]
    apply: bool,
...
fn main() {
    ...
    let existing: Vec<Product> = if cli.data_file.exists() {
        let raw = fs::read_to_string(&cli.data_file).expect("Failed to read productos.json");
    ...
    if cli.json {
        ...
        if report.added.is_empty() && report.removed.is_empty() && report.updated.is_empty() {
            println!("✓ productos.json is in sync with product pages.");
        }
    }
    ...
    if cli.apply {
        ...
        fs::write(&cli.data_file, json_str).expect("Failed to write productos.json");

        if !cli.json {
            println!("✓ productos.json updated ({} products).", merged.len());
        }
    } else if !cli.json && !report.added.is_empty() {
        println!("Run with --apply to write changes to productos.json");
    }
}
