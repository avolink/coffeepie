...
All amounts in COP (Colombian Pesos). Credits (Cr) conversion:
  1 Cr ≈ 1 COP (subject to regional pricing adjustments)
"""
...
def credits_to_cop(cr: int) -> int:
    """Convert Credits to COP (for display only)."""
    return cr


def cofp_to_cop(cofp: int) -> int:
    """Convert COFP to COP. 1 COFP = 1'000 COP."""
    return cofp * 1_000


def cofp_to_credits(cofp: int) -> int:
    """Convert COFP to Credits. 1 COFP = 1'000 Cr."""
    return cofp * 1_000
