...
function parsePrice(priceStr) {
    ...
    try {
        // Handle new format: 75'000 COP (also backward compat with $1.200.000,00)
        let clean = priceStr.toString()
            .replace(/COP/gi, '')    // Remove currency suffix
...
function updateCartUI() {
    ...
    const squarishCartPaths = `
        <path fill="currentColor" d="M197.9 55.9L169.9 127.4 64.5 127.4 27.6 29.8 0 29.8 0.2 16.7 36.5 16.7 73.4 114.3 160.9 114.3 183 55.9"></path>
        <circle fill="currentColor" cx="143.8" cy="153" r="13"></circle>
        <circle fill="currentColor" cx="90.8" cy="153" r="13"></circle>
    `;

    document.querySelectorAll('a[href*="carrito"], a[href*="/cart"], a[href*="cart-page"], [data-hook="cart-icon-button"], [data-testid="cart-link"]').forEach(link => {
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('a[href*="cart-page"]').forEach(l => l.href = '/cart.html');
    updateCartUI();
    initCartPage();
});
...
function renderCartPage() {
    ...
    if (!container) {
        // Clean vanilla page: use #cart-container
        var cleanContainer = document.getElementById('cart-container');
        if (cleanContainer) {
    ...
    // Prevent Arabic translation on cart pages — keep English layout
    container.setAttribute('data-cp-no-translate', '');

    var t = function(key) {
        if (window.CoffeePieLang && typeof window.CoffeePieLang.translate === 'function') {
            var translated = window.CoffeePieLang.translate(key, window.CoffeePieLang.get ? window.CoffeePieLang.get() : 'es');
    ...
    if (cart.length === 0) {
        container.innerHTML = `
            <div style="text-align:center; padding:100px 20px; font-family: Arial, Helvetica, sans-serif; background:#fff; min-height: 60vh;">
                <h2 style="font-size:2rem; color:#000; font-weight:400;">${t('Mi carrito')}</h2>
                <p style="margin:20px 0; color:#666; font-size:1.1rem;">${t('Tu carrito está vacío.')}</p>
                <a href="/store" style="display:inline-block; padding:18px 40px; background:#000; color:#fff; text-decoration:none; border-radius:30px; font-size:1.1rem; cursor:pointer; font-weight:normal; letter-spacing:0.5px; text-align:center;">${t('Sigue navegando')}</a>
            </div>
        `;
        return;
    }
    ...
    cart.forEach((item, index) => {
        ...
        let productUrl = '/store.html';
        
        // First try URL-based matching
        if (item.url && item.url !== 'tienda' && item.url !== 'store.html') {
            if (item.url.startsWith('http://') || item.url.startsWith('https://')) {
                productUrl = item.url;
            } else if (item.url.startsWith('/')) {
                productUrl = item.url;
            } else if (item.url.startsWith('products/')) {
                productUrl = '/' + item.url;
            } else if (item.url.includes('/')) {
                productUrl = '/' + item.url;
            } else {
                productUrl = '/products/' + item.url;
            }
        } 
        // Then try name-based matching as fallback
            ...
            if (nameLower.includes('commander') && nameLower.includes('basic')) {
                productUrl = '/products/terminal-codec-commander-basic-by-coffee-pie';
            } else if (nameLower.includes('commander') && nameLower.includes('core')) {
                productUrl = '/products/terminal-codec-commander-core-by-coffee-pie';
            } else if (nameLower.includes('commander') && nameLower.includes('pro')) {
                productUrl = '/products/terminal-codec-commander-pro-by-coffee-pie';
            } else if (nameLower.includes('framework') && nameLower.includes('usb-c')) {
                productUrl = '/products/usb-c-expansion-card-by-framework';
            } else if (nameLower.includes('framework') && nameLower.includes('usb-a')) {
                productUrl = '/products/usb-a-expansion-card-by-framework';
            } else if (nameLower.includes('framework') && nameLower.includes('hdmi')) {
                productUrl = '/products/hdmi-expansion-card-by-framework';
            } else if (nameLower.includes('framework') && nameLower.includes('displayport')) {
                productUrl = '/products/displayport-expansion-card-by-framework';
            } else if (nameLower.includes('framework') && nameLower.includes('audio')) {
                productUrl = '/products/audio-expansion-card-by-framework';
            } else if (nameLower.includes('framework') && nameLower.includes('sd')) {
                productUrl = '/products/sd-expansion-card-by-framework';
            } else if (nameLower.includes('framework') && nameLower.includes('microsd')) {
                productUrl = '/products/micro-sd-expansion-card-by-framework';
            } else if (nameLower.includes('framework') && nameLower.includes('storage') && nameLower.includes('1tb')) {
                productUrl = '/products/storage-expansion-card-1tb-by-framework';
            } else if (nameLower.includes('framework') && nameLower.includes('storage') && nameLower.includes('250')) {
                productUrl = '/products/storage-expansion-card-250gb-by-framework';
            } else if (nameLower.includes('framework') && nameLower.includes('touchpad')) {
                productUrl = '/products/touchpad-module-by-framework';
            } else if (nameLower.includes('framework') && nameLower.includes('numpad')) {
                productUrl = '/products/numpad-module-for-commander-by-framework';
            } else if (nameLower.includes('tp-link') && nameLower.includes('wifi')) {
                productUrl = '/products/wifi-adapter-by-tp-link';
            } else if (nameLower.includes('tp-link') && nameLower.includes('ethernet')) {
                productUrl = '/products/ethernet-rj45-adapter-by-tp-link';
            } else if (nameLower.includes('ugreen') && nameLower.includes('wifi') && nameLower.includes('bluetooth')) {
                productUrl = '/products/wifi-and-bluetooth-adapter-by-ugreen';
            } else if (nameLower.includes('lofree')) {
                productUrl = '/products/custom-keycaps-set-for-commander-by-lofree';
            } else if (nameLower.includes('womier')) {
                productUrl = '/products/custom-keycaps-set-for-commander-by-womier';
            } else if (nameLower.includes('cherry') && nameLower.includes('stabilizers')) {
                productUrl = '/products/low-profile-stabilizers-set-for-commander-by-cherry';
            } else if (nameLower.includes('keycaps') && nameLower.includes('low')) {
                productUrl = '/products/low-profile-keycaps-set-for-commander';
            } else if (nameLower.includes('powerowl')) {
                productUrl = '/products/hot-swappable-battery-for-commander-by-powerowl';
            } else if (nameLower.includes('keychron')) {
                productUrl = '/products/optomechanical-switches-for-commander-by-keychron';
            } else if (nameLower.includes('razer')) {
                productUrl = '/products/optomechanical-switches-for-commander-by-razer';
            } else if (nameLower.includes('xicom')) {
                productUrl = '/products/rj45-to-sfp-optical-fiber-converter-by-xicom';
            } else if (nameLower.includes('tpe')) {
                productUrl = '/products/tpe-optomechanical-switches-by-coffee-pie';
            } else if (nameLower.includes('commander') && nameLower.includes('base')) {
                productUrl = '/products/commander-base-heatsink';
            } else if (nameLower.includes('commander') && nameLower.includes('cover')) {
                productUrl = '/products/commander-keyboard-cover';
            } else if (nameLower.includes('commander') && nameLower.includes('back') && nameLower.includes('io')) {
                productUrl = '/products/commander-back-io-panel';
            } else if (nameLower.includes('commander') && nameLower.includes('keyboard') && nameLower.includes('module')) {
                productUrl = '/products/commander-keyboard-module-w-micro';
            }
        }
        ...
        itemsHtml += `
            <div class="cart-item-row">
                <div class="cart-item-info">
                    <a href="${productUrl}"><img src="${escapeHtml(item.image)}" class="cart-item-image" onerror="this.src='/assets/avo/media/557674_3c3a2d29a9434c33a754d6cac7b98b98.png';"></a>
                    <div class="cart-item-details">
                        <h3 class="cart-item-title"><a href="${productUrl}">${escapeHtml(item.name)}</a></h3>
                        <p class="cart-item-unit-price">${p.toLocaleString('de-CH')} COP</p>
                        <p class="cart-item-variant">${escapeHtml(item.variant || 'Selección estándar')}</p>
                    </div>
                </div>
                <div class="cart-item-actions">
                    <div class="cart-item-controls-wrapper">
                        <div class="cart-item-controls">
                            ...
                            <input type="number" class="avo-cart-qty-input" value="${item.quantity}" min="1" max="999" onchange="setQty(${index}, this.value, event)">
                            <button type="button" onclick="updateQty(${index}, 1, event)" class="qty-btn">+</button>
                        </div>
                    ...
                    <div class="cart-item-price">
                        ${(p * item.quantity).toLocaleString('de-CH')} COP
                    </div>
                </div>
            </div>
        `;
    ...
    container.innerHTML = `
        ...
        <div class="cart-page-container">
            ...
            <div class="cart-items-section">
                <h1 style="font-size:2rem; font-weight:400; margin-bottom:40px;">${t('Mi carrito')}</h1>
                ...
                <div class="cart-promo-notes">
                    <div class="promo-note-btn">
                        ...
                        <span>${t('Ingresar código promocional')}</span>
                    </div>
                    <div class="promo-note-btn">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
                        <span>${t('Agregar una nota')}</span>
                    </div>
                </div>
            </div>

            <div class="cart-summary-section">
                <h2 style="font-size:1.5rem; font-weight:400; margin-bottom:35px;">${t('Resumen del pedido')}</h2>
                ...
                <div style="border-top:1px solid #e0e0e0; padding-top:25px; margin-bottom:25px;">
                    <div style="display:flex; justify-content:space-between; margin-bottom:15px; font-size:1.05rem;">
                        ...
                        <span>${total.toLocaleString('de-CH')} COP</span>
                    </div>
                ...
                <button onclick="window.location.href='/secure-payment'" style="width:100%; padding:18px; background:#000; color:#fff; border:none; border-radius:30px; font-size:1.1rem; cursor:pointer; margin-bottom:15px; font-weight:normal; letter-spacing:0.5px;">
                    ${t('Pago Seguro')}
                </button>
...
function updateAvoTotals(total) {
    const s = `${total.toLocaleString('de-CH')} COP`;
...
function showFeedback(name) {
    ...
    const translatedAgregado = window.CoffeePieLang && typeof window.CoffeePieLang.translate === 'function'
        ? window.CoffeePieLang.translate('agregado', window.CoffeePieLang.get ? window.CoffeePieLang.get() : 'es')
        : 'agregado';
    t.innerText = `${name} ${translatedAgregado || 'agregado'}`;
...
document.addEventListener('DOMContentLoaded', () => {
    const quantityInput = document.querySelector('.avoui-product-quantity-container input[type="number"], [class*="product-quantity"] input[type="number"], .cp-product__qty-input');
    if (quantityInput) {
        quantityInput.min = '1';
...
document.addEventListener('click', (e) => {
    ...
    if (btn) {
        ...
        if (btn.classList.contains('cp-product__add-btn--go-to-cart')) {
        ...
        let priceEl = [...(container?.querySelectorAll('span, p') || [])].find(s => s.innerText.includes('COP'));
        if (!priceEl) {
            priceEl = [...document.querySelectorAll('span, p')].find(s => s.innerText.includes('COP'));
        }
        // Fallback: get from JSON-LD schema
        if (!priceEl) {
            const jsonLd = document.querySelector('script[type="application/ld+json"]');
            if (jsonLd) {
                try {
                    ...
                    if (offers) {
                        ...
                        if (priceValue) {
                            priceEl = { innerText: { trim: () => parseInt(priceValue).toLocaleString('de-CH') + ' COP' } };
                        }
                    }
                } catch (e) {}
            }
        ...
        if (nameEl && priceEl) {
            ...
            if (!pageUrl || pageUrl === 'tienda') {
                const productLink = container?.querySelector('a[data-hook="product-link"], a[class*="product"], a[href*="products"]');
                if (productLink) {
            ...
            if (currentRadio) {
                ...
                if (wrapper) {
                    const variantPriceEl = [...wrapper.querySelectorAll('span, p, div')].find(s => s.innerText && s.innerText.includes('COP'));
                    if (variantPriceEl) {
            ...
            if (ex) {
                ...
                if (ex.quantity > 999) ex.quantity = 999;
            } else {
                if (qtyToAdd > 999) qtyToAdd = 999;
                cart.push({ ...prod, quantity: qtyToAdd });
            }
            saveCart(cart);
            showFeedback(prod.name);
            
            // Add 'Agregado!' effect to the button itself
            const textTarget = btn.querySelector('span') || btn;
            const originalInner = textTarget.textContent;
            const originalBg = btn.style.backgroundColor;
            const originalColor = textTarget.style.color;
            const originalBtnColor = btn.style.color;

            btn.classList.add('cp-product__add-btn--added');
            btn.style.setProperty('background-color', '#1a8a3f', 'important');
            btn.style.setProperty('color', '#ffffff', 'important');
            textTarget.style.setProperty('color', '#ffffff', 'important');
            
            const agregadoText = window.CoffeePieLang && typeof window.CoffeePieLang.translate === 'function'
                ? window.CoffeePieLang.translate('Agregado!', window.CoffeePieLang.get ? window.CoffeePieLang.get() : 'es')
                : null;
            textTarget.textContent = agregadoText || 'Agregado!';

            setTimeout(() => {
                btn.classList.remove('cp-product__add-btn--added');
                btn.style.backgroundColor = originalBg;
                btn.style.color = originalBtnColor;
                textTarget.style.color = originalColor;
                
                const translated = window.CoffeePieLang && typeof window.CoffeePieLang.translate === 'function'
                    ? window.CoffeePieLang.translate('Ir al carrito', window.CoffeePieLang.get ? window.CoffeePieLang.get() : 'es')
                    : null;
                const irAlCarritoText = translated || 'Ir al carrito';
                textTarget.textContent = irAlCarritoText;
                
                // Prevent the translation engine from reverting it back to "Agregar al Carrito"
                [textTarget, btn].forEach(el => {
                    if (el.hasAttribute('data-translate')) el.setAttribute('data-translate', 'Ir al carrito');
                    if (el.hasAttribute('data-cp-original')) el.setAttribute('data-cp-original', 'Ir al carrito');
                });
                if (btn.hasAttribute('aria-label')) btn.setAttribute('aria-label', 'Ir al carrito');
                
                btn.classList.add('cp-product__add-btn--go-to-cart');
            }, 1000);
        }
    }
});
...
// Re-render cart when CoffeePieLang dictionary loads or language changes
(function hookLangRefresh() {
    if (!window.CoffeePieLang || !window.CoffeePieLang.set) {
        setTimeout(hookLangRefresh, 100);
        return;
    }
    var _reentrant = false;
    var _origSet = window.CoffeePieLang.set;
    window.CoffeePieLang.set = function(lang) {
        _origSet(lang);
        if (_reentrant) return;
        var onCartPage = window.location.pathname.includes('carrito') || window.location.pathname.includes('cart');
        if (onCartPage && (document.getElementById('custom-cart-root') || document.getElementById('cart-container'))) {
            _reentrant = true;
            renderCartPage();
            _reentrant = false;
        }
    };
    var _origRefresh = window.CoffeePieLang.refresh;
    window.CoffeePieLang.refresh = function() {
        _origRefresh();
        if (_reentrant) return;
        var onCartPage = window.location.pathname.includes('carrito') || window.location.pathname.includes('cart');
        if (onCartPage && (document.getElementById('custom-cart-root') || document.getElementById('cart-container'))) {
    ...
    // Also re-render when lang.js finishes loading the dictionary (cplangchange event)
