...
(function () {
    ...
    var SUPPORTED = ['es', 'en', 'pt', 'fr', 'de', 'el', 'ja', 'ko', 'zh', 'ru', 'ar', 'hi'];
    var DEFAULT = 'es';
    ...
    function translateElement(el, toLang) {
        ...
        if (el.closest('[data-cp-no-translate], [translate="no"], .notranslate')) return;
    ...
    function applyLanguage(lang, isRefresh) {
        if (!dict || !fastLookup) return;
        if (!isRefresh && lang !== currentLang) {
            currentLang = lang;
        }
        ...
        // Notify listeners (e.g. vanilla-gallery) to re-render ONLY if it's not a background refresh
        if (typeof CustomEvent !== 'undefined' && !isRefresh) {
    ...
    function injectRTLCSS() {
        ...
        style.innerHTML = `
            ...
            .lang-ar p, .lang-ar h1, .lang-ar h2, .lang-ar h3, .lang-ar h4, .lang-ar h5, .lang-ar h6, .lang-ar li, .lang-ar a, .lang-ar span, .lang-ar button, .lang-ar label {
    ...
    window.CoffeePieLang = {
        ...
        refresh: function () { applyLanguage(currentLang, true); },
        supported: SUPPORTED,
