...
3) Use aphostrophe "'" as Thousands Separator for presentation to human users, dot "." as decimal separator, comma "," as prhases separator in strings and paragraphs. All dates should be stored in YYYY-MM-DD format for better sorting.
...
- Main Contributors: avolink, juanvelezmunoz32-ai, Diego-M-PZ, torvalds?, linuxfoundation?, valve?, microsoft?, google?, nvidia?, amd?, intel?, arm?...
...
- COFFEE Token — COFP (TRC-20): The native utility token of the Coffee Pie® ecosystem, deployed on the TRON blockchain (TRC-20 standard) for ultra-low fees and fast settlement. Smart contract: `blockchain/COFP_Token.sol` — deployment guide at `blockchain/DEPLOY.md`. **Elastic supply model**: initial supply of 100'000'000 COFP minted at deployment. New tokens are emitted at a rate of 1 COFP per 1 Slice served for 1 minute by Providers on the QFDM Network — supply grows organically with the network, backed by real compute provision. No supply cap. **Contract governance**: ownership must be transferred to a Gnosis Safe multi-sig wallet (minimum 4/7 signers: Core Dev Lead, Legal Counsel, Community Representative, CFO, cold storage key, External Auditor, Emergency Trustee) with a 48-hour timelock on all administrative functions. This is mandatory before BVC stock exchange listing — no single individual may hold unilateral control over minting, pausing, or ownership transfer.

- **COFP Token Unit**: **1 COFP = 1 Coffee Pie® Slice served for 1 minute** (1 Slice·min). This is the fundamental accounting unit of the QFDM Network. Earnings scale linearly: a Provider serving 4 Slices simultaneously for 60 minutes earns 4 × 60 = 240 COFP. This unit is enforced by the Coffee Pie® backend ledger, not by the smart contract. **Two-currency model**: COFP is exclusively for Providers and Contributors — consumers (end users) never hold or interact with COFP directly. Consumers operate entirely with Coffee Pie® Credits (Cr), obtained by watching Ads (paid by Advertisers) or purchasing Credit Packages at `coffeepie.co/prices`. Credits are the only currency consumers spend to rent computing Slices.

- **Provider Fiat Settlement Tiers**: When Providers burn COFP for fiat, the amount received = base COFP price × (1 + tier margin). The global base cost is 0.29 COP/COFP (approx 0.000069 USD) per governance vote; all regional pricing derivatives are tracked in `avgSliceCost.json`. Tiers reward infrastructure quality, reliability, and environmental responsibility — the higher the Tier, the better the margin. Full requirements at `PROVIDERS.md` and `coffeepie.co/proveedores-nube`.
...
- COFP tokens confer different rights depending on how they were acquired:
  ...
  * Contributors (Community Developers, Moderators, Translators, Auditors, Tech Supporters, etc.): Earn COFP by contributing to the ecosystem (code, documentation, translations, moderation, community support, etc.). Rights: vote on technical decisions (development stack, deprecation policies, implementation priorities). Wallet holding limit: **10% of the total COFP supply per wallet**, enforced by the Coffee Pie® backend, not the smart contract. Contributors may: (a) sell earned COFP on the TRON open market (TRC-20) — the seller permanently loses all voting rights, the buyer receives an Investor-class token with economic rights only; (b) hold COFP for long-term valuation appreciation and future dividends after IPO; (c) burn COFP for Coffee Pie® Credits at a rate of **10 Cr per COFP** to consume computing resources on the QFDM Network. **Contributors cannot burn COFP for fiat** — that right is exclusive to Providers. Contributors do not spend Credits to consume computing resources; they can access service capacity through standard consumer Credit channels if needed.
...
- **Two-Currency Model**: Coffee Pie® operates two strictly separate currencies. **COFP** (TRC-20 on TRON) is the supply-side token: only Providers and Contributors earn it, and only Providers may burn it for fiat cash flow (transferred to their registered bank account within 24-72 hours) to sustain datacenter operations. **Credits (Cr)** are the demand-side consumer currency: end users obtain Cr exclusively by watching Ads (paid by Advertisers) or purchasing Credit Packages at `coffeepie.co/prices`, then spend Cr to rent computing Slices. Consumers never hold, earn, or interact with COFP. Contributors may burn COFP for Credits at a rate of **10 Cr per COFP**. The COFP burn-for-fiat mechanism is an internal ledger operation exclusive to Trusted Providers — it is not a public token redemption.

- Transparent Monetization Model: 1) QFDM Royalty Fees for the rental of codec terminals from manufacturers to third parties 2) sale of massive highly segmented advertising spaces to Advertisers 3) alliances with the public sector, educational entities, libraries, NGOs, etc. 4) services to manufacturers such as setup, maintenance, repair, recycling and final disposal 5) direct consumers who do not wish to watch ads or have a need for high computing resources, but prefer the convenience of our service instead of buying a "NASA computer".
...
- Numeric Format: The dotsymbol '.' is the standard decimal separator, and the comma symbol ',' should also be detected as a thousand separator when working with numbers (floats), but in order to avoid confussions and missliding data in DBs, we suggest using comma ',' for strings only, e.g.: pi = 3.141592653589793. Please always use apostrophe for final user presentation "'", and underscore '_' if needed as thousands separators when programming for better technical contributors readability, which is the Rust standard, eg.: int = Frontend: 1'000'000 units | Frontend: float = 7'837,50 | Programming: float = 25_500_000.43 | etc. This way it can be easily swapped the underscore for apostrophe for better representation facing the end user and non-technical contributors.
...
- Tier Opacity: Provider infrastructure Tier (I–V) is a SUPPLY-SIDE internal signal — assigned/verified by Coffee Pie at onboarding (never self-reported, since a higher Tier earns a higher settlement margin) and NEVER exposed to consumers. Consumers see only the approximate region (city, e.g. Sabaneta/Medellín/Bogotá) and latency/ping (≈10–80 ms expected). Which Tier serves a workload is the offer side's concern, to adapt to demand/market conditions. No consumer-facing API or UI may surface `Tier` or node identity; the broker's consumer response carries `{ region, ping_ms }`, never `{ tier, node }`. Placement is graceful degradation (per-class tier-preference within a latency band), not rigid class→Tier binding. See `SCHEDULING.md` and the `coffeepie-scheduler` crate.
...
Decision: TLS verification bypass limited to internal L2/L3 private networks.
  ...
  Documented in: `coffeepie_orchestrator/actor/crates/shared/src/tls/noverify.rs`, `coffeepie_orchestrator/tunnel-server/src/broker/mod.rs`

Decision: Post-Quantum KEM handled by Rust (libcrux), Python pqcrypto is deprecated.
  Reason: The Python `pqcrypto` package (`coffeepie_orchestrator/server/requirements.txt`) is 
