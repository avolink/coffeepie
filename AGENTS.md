...
- COFFEE Token — COFP (TRC-20): The native utility token of the Coffee Pie® ecosystem, deployed on the TRON blockchain (TRC-20 standard) for ultra-low fees and fast settlement. Smart contract: `blockchain/COFP_Token.sol` — deployment guide at `blockchain/DEPLOY.md`. **Contract governance**: ownership must be transferred to a Gnosis Safe multi-sig wallet (minimum 4/7 signers: Core Dev Lead, Legal Counsel, Community Representative, CFO, cold storage key, External Auditor, Emergency Trustee) with a 48-hour timelock on all administrative functions. This is mandatory before BVC stock exchange listing — no single individual may hold unilateral control over minting, pausing, or ownership transfer. COFP tokens confer different rights depending on how they were acquired:

  * Contributors (Community Developers): Earn COFP by contributing code, documentation, translations, or community support. Rights: vote on technical decisions (development stack, deprecation policies, implementation priorities); burn tokens for Platform Credits to consume computing power, capped at **100'000 COFP per wallet per month** to prevent a single contributor from draining network capacity during peak hours. Wallet holding limit: **1'000'000 COFP per wallet** (1% of the 100M total supply). These limits are enforced by the Coffee Pie® backend, not the smart contract. **Important**: selling earned COFP on the open market is permitted, but the seller permanently loses all voting rights — the buyer receives an Investor-class token with economic rights only (dividends, valuation), no governance vote.
