...
AI training, AI inferencing, Big Data, deploying servers, apps, databases, studying, working, writing, developing, programming, compiling, designing, composing, transforming, editing, exporting, rendering, calculating, organizing, communicating, collaborating, starting a business, selling, analysing, researching, creating, entertaining, casual gaming, but not really meant for hardcore gaming due to higher latency (30 ~ 80ms), you'd enjoy competitive games better with a handheld or console, at least until we manage to lower down latencies to ≥10ms average, which is the long term goal.<br>
...
ARM-based SBCs (Radxa, Orange Pi, custom RISC-V, etc) running:<br>
  - Operating System: Armbian / Debian Minimal (CLI).<br>
...
💰 Monetization & Ethics:<br>
 ...
 1. Free Tier: Advertisers pay for the computing power, ideal for casual users (Ads on-demand). Have as many Slices as you want — but a dormant Slice (powered off or suspended) still reserves SSD/HDD on a provider's node, so a small **Parking Fee** of 10 Cr per dormant Slice per hour applies from the 5th dormant Slice and up (the first 4 are parking free). It only covers the providers' storage cost plus their fair margin, and keeps idle "zombie" machines from hoarding storage — saving the planet, not the bots. See `PROVIDERS.md`.<br>
 &nbsp;&nbsp;&nbsp;**Free-tier Credits & Rush Hour Balancing:** Free-tier Credits are *earned by watching Ads* — the Advertiser's bid equals the Cr credited to the user **1:1** (no conversion markup, to keep taxation simple). Ad-earned Cr **expire 1 hour after being granted**; unused Cr are forfeited (e.g. watch 3 Ads = 900 Cr, run a machine costing 700 Cr, and the remaining 200 Cr are burned). Free machines consume **30 Cr/slice·min off-peak**; during rush-hour windows (e.g. 07:00–09:00 and 19:00–22:00 local — regional/governance-tunable) the rate scales with live regional utilization up to **~1'000 Cr/slice·min** — peak use simply costs *more Ads*, nudging casual users to off-peak (framed as an off-peak reward, not a penalty). **Paid packages are never surged** and carry longer validity (**1 week / 1 month / 1 year**). Rush Hour is **customer-side only**: providers keep earning 1 COFP/slice·min and benefit only *indirectly* via the governance + post-IPO dividend/valuation rights earned COFP carries (a utility token, not a security). **Basic access is never denied** — at peak the Free tier degrades gracefully (more Ads, lower priority), it is not priced out.<br>
